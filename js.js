...
function createGrid(value) {
	...
	for (let i = 0; i < value; i++) {
		...
		for (let j = 0; j < value * 1.5; j++) {
			...
			gridContainer.lastChild.lastChild.style.backgroundColor =
...
monetModeButtonsArray.forEach((button) =>
	monetModeButtonsFragment.appendChild(button)
);
...
function resetGridColor() {
	gridSquares.forEach(
		(square) =>
			(square.style.backgroundColor = gridContainer.style.backgroundColor)
	);
}
...
lighten.addEventListener('click', (e) =>
	addShadeListener(lightenPercentage, intervalRate)
);
darken.addEventListener('click', (e) =>
	addShadeListener(darkenPercentage, intervalRate)
);
...
function shadeOnMousedown(percentage, intervalRate) {
	gridSquares.forEach((square) =>
		square.addEventListener('mousedown', (e) => {
			mouseIsDown = true;
			square.style.backgroundColor = shadeColor(
